...
if SIS_RELEASE_ALLOW_DIRTY=1 \
  ...
  ./scripts/release-readiness.sh v0.0.0-rc.1 >"${pending_out}" 2>&1; then
  echo "release-readiness-smoke: rc precheck with pending record unexpectedly passed" >&2
  cat "${pending_out}" >&2
  exit 1
fi

if ! grep -q 'release-candidate-check: v0.0.0-rc.1 is blocked' "${pending_out}"; then
  echo "release-readiness-smoke: pending rc record did not run release-candidate check" >&2
  cat "${pending_out}" >&2
  exit 1
fi
...
SIS_RELEASE_ALLOW_DIRTY=1 \
  ...
  ./scripts/release-readiness.sh v0.0.0-rc.1 >"${complete_out}" 2>&1

if ! grep -q 'release-candidate-check: v0.0.0-rc.1 has recorded production validation evidence' "${complete_out}"; then
  echo "release-readiness-smoke: complete rc record did not pass release-candidate check" >&2
  cat "${complete_out}" >&2
  exit 1
fi
...
SIS_RELEASE_ALLOW_DIRTY=1 \
  ...
  SIS_RELEASE_SKIP_REMOTE_TAG_CHECK=1 \
  SIS_RELEASE_VALIDATION_RECORD="${pending_record}" \
  ./scripts/release-readiness.sh v0.0.0 >"${stable_out}" 2>&1

if grep -q 'release-candidate-check:' "${stable_out}"; then
  echo "release-readiness-smoke: stable precheck unexpectedly ran release-candidate check" >&2
  cat "${stable_out}" >&2
  exit 1
fi
