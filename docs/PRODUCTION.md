...
for each KV row, and uses normalized `clients`, `sessions`, `custom_lists`, and `stats`
tables for client/session/list/stats CRUD while preserving portable KV payloads for
backup/export. It runs `PRAGMA quick_check` during store verification. Prefer it for
deployments that expect more client, session, custom-list, stats, or config-history churn.
