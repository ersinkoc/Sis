...
- SPEC §19-style DNS acceptance coverage now exists in `internal/dns/acceptance_test.go`, using fake DoH upstreams and real UDP/TCP DNS clients for default forwarding/blocking, allowlist override, active/inactive schedules, upstream failover, cache-hit logging, hashed privacy logging, and per-client rename/group move. It is not yet the full 10-scenario suite.
...
| Tests | SPEC §19 acceptance suite is partial | Extend fake-DoH/real-client coverage to remaining scenarios: hot reload, restart persistence | 6-12h |
