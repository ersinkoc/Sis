...
- SPEC §19-style DNS acceptance coverage now exists in `internal/dns/acceptance_test.go`, using fake DoH upstreams and real UDP/TCP DNS clients for default forwarding/blocking, allowlist override, active/inactive schedules, upstream failover, cache-hit logging, and hashed privacy logging. It is not yet the full 10-scenario suite.
...
| Tests | SPEC §19 acceptance suite is partial | Extend fake-DoH/real-client coverage to remaining scenarios: per-client rename, hot reload, restart persistence | 8-16h |
